...
def hand_detection(landmark_list):
...
def detect_gesture(frame, landmark_list, processed):
    if len(landmark_list) >= 21:
        ...
        if hand_detection(landmark_list):
            x = int(index_finger_tip.x * screen_width)
            y = int(index_finger_tip.y * screen_height)
            cv2.circle(frame, (x, y), 10, (0, 0, 255), -1)  
            cv2.putText(frame, "Hand_closed", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        if util.get_distance([landmark_list[4], landmark_list[5]]) < 50 and util.get_angle(landmark_list[5],
                                                                                             landmark_list[6],
                                                                                             landmark_list[8]) > 90:
